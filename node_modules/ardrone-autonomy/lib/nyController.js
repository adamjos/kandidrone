...
function Controller(client, options) {
    ...
    this._lastTag = {x: 0, y: 0};


    // Configure the four PID required to control the drone
    this._pid_x   = new PID(0.5, 0.01, 0.35);
    this._pid_y   = new PID(0.5, 0.01, 0.35);
    this._pid_z   = new PID(0.8, 0.1, 0.35);
...
Controller.prototype.hovertwo = function() {
    this._go({x: this._goal.x, y: this._goal.y, z: this._goal.z, yaw: this._state.yaw});
}
...
Controller.prototype._processNavdata = function(d) {
    ...
    pitch = d.demo.frontBackDegrees;
    roll = d.demo.leftRightDegrees;
    ...
    if (d.visionDetect && d.visionDetect.nbDetected > 0) {
        ...
        var tagPos = {x: this._state.x + measured.x  - (this._state.z * Math.tan(pitch)),
                      y: this._state.y + measured.y - (this._state.z * Math.tan(roll))};
                     //  - (this._state.z*Math.tan(pitch)),



        //if (Math.sqrt(((tagPos.x - this._lastTag.x)^2)+((tagPos.y - this._lastTag.y)^2)) > EPS_TAG) {
        if ((i === 0) && (this._lastTag.x === 0) && (this._lastTag.y === 0)) {
            this._lastTag = tagPos;
            this.emit('Tag Detected', tagPos);
            this._goal.x = tagPos.x;
            this._goal.y = tagPos.y;
            i = i+1;
            value = {z: this._state.z, p: pitch, r: roll};

            this.emit('Got values', value);

        }
        else if((i === 1) && (Math.abs(tagPos.x - this._lastTag.x) > EPS_TAG1) && (Math.abs(tagPos.y - this._lastTag.y) > EPS_TAG1)) {
            this._lastTag = tagPos;
            this.emit('Tag Detected', tagPos);
            this._goal.x = tagPos.x;
            this._goal.y = tagPos.y;
        ...
        else if ((i === 2) && (Math.abs(tagPos.x - this._lastTag.x) < EPS_TAG1) && (Math.abs(tagPos.y - this._lastTag.y) < EPS_TAG1)) {
            this._lastTag = tagPos;
            this.emit('Tag Detected', tagPos);
            this._goal.x = tagPos.x;
            this._goal.y = tagPos.y;
        ...
        //this._goal.x = this._tag.x;
        //this._goal.y = this._tag.y;
